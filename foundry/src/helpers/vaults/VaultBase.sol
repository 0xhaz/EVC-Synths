...
import {EVCClient, IEVC} from "src/helpers/utils/EVCClient.sol";
...
abstract contract VaultBase is IVault, EVCClient {
    ...
    modifier nonReentrant() virtual {
        if (reentrancyLock == REENTRANCY_LOCKED) {
            revert Reentrancy();
        }
    ...
    function createVaultSnapshot() internal {
        ...
        // taken before any action is taken on the vault that affects the vault asset records and deleted at the end,
    ...
    /// @notice Checks the account status
    /// @dev Must be overridden by child contracts
    function doCheckAccountStatus(address account, address[] calldata) internal view virtual;

    /// @notice Disables a controller for an account
    /// @dev Must be overridden by child contracts. Must call the EVC.disableController() only if it's safe to do si
