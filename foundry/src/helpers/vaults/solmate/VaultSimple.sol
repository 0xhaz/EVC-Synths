...
import {VaultBase, IEVC, EVCClient} from "src/helpers/vaults/VaultBase.sol";

/**
 * @title VaultSimple
 * @dev It provides basic funcionality for vault
 * @notice In this contract, the EVC is authenticated before any action that may affect the state of the vault or an
 * account.
 * This is done to ensure that if it's EVC calling, the account is correctly authorized. Unlike solmate,
 * VaultSimple implementation prevents from share inflation attack by using virtual assets and shares. Look
 * into OpenZeppelin's documentation for more information. This vault implements internal balance tracking. This
 * contract does not take
 * the supply cap into account when calculating max deposit and max mint values.
 */
contract VaultSimple is VaultBase, Owned, ERC4626 {
    ...
    function doCreateVaultSnapshot() internal virtual override returns (bytes memory) {
        // maake total assets the snapshot and return it
    ...
    /// @param oldSnapshot The snapshot of the vault state before the action
    ...
    /// @dev That function is manipulated in its current form as it uses exact values. Considering that other vaults may
    ...
    function convertToShares(
        uint256 assets
    ) public view virtual override nonReentrantRO returns (uint256) {
    ...
    /// @notice Simulates the effects of withdrawing a certain amount of shares at the current block
    /// @param assets The amount of shares to simulate withdrawing
    /// @return The amount of assets that would be burned
    ...
    function transfer(address to, uint256 amount) public virtual override callThroughEVC nonReentrant returns (bool) {
        ...
        // added here so that if anyone changes the snapshot/vault status check mechanism in the inheriting contracts
    ...
    function deposit(
        ...
        require((shares = _convertToShares(assets, false)) != 0, "Rounding error");

        // Need to transfer before minting or ERC777s could re-enter
    ...
    function mint(
        ...
        assets = _convertToAssets(shares, true); // no need to check for rounding error since we round up in previewMint
    ...
    /// @notice Withdraws a certain amount of shares for a receiver
    ...
    function withdraw(
        ...
        address receiver,
        address owner
    ) public virtual override callThroughEVC nonReentrant onlyEVCAccountOwner returns (uint256 shares) {
        address msgSender = _msgSender();

        createVaultSnapshot();

        shares = _convertToShares(assets, true); // no need to check for rounding error since we round up in
            // previewWithdraw
    ...
    function redeem(
        ...
        address receiver,
        address owner
    ) public virtual override callThroughEVC nonReentrant onlyEVCAccountOwner returns (uint256 assets) {
        address msgSender = _msgSender();
        ...
        require((assets = _convertToAssets(shares, false)) != 0, "Rounding error");
