 ...
 * as per the tipReceiver value. For that, the tip amount should be transferred to this contract during:
 * - the submission step (using signed calldata of the EVC permit functionality) in order to tip the submitter
 * - the execution step (either while executing non-EVC or EVC operations) in order to tip the executor
 * The OrderOperator will always use the full amount of its balance at the time for the tip payouts.
 * Important: the submitter/executor must set the value of the tipReceiver variable to the address
 * where they want to receive the tip. For safety, it should happen atomically during the EVC batch call,
 * before the actual submission/execution of the order.
...
contract LightweightOrderOperator {
    ...
    function execute(
        ...
        // execute EVC operations, i.e interact with vaults
    ...
    /// @notice Cancels an order that is either new or pending
    ...
    function _batch(
        NonEVCBatchItem[] calldata operations
    ) internal {
    ...
    /// @param tipToken The token to be used for the tip
    function _payoutTip(
        ERC20 tipToken
    ) internal {
    ...
    function _verifyOrder(
        ...
        // verify that the non-EVC operations contain only operations that do not require authentication
